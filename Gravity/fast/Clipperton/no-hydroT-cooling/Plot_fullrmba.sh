...
#===== Input parameters' value change=====
    ...
    gmt makecpt -C../../../vikO.cpt+h -T10/55/5 -Z >grav_therm.cpt
    gmt makecpt -C../../../basecpt_grav.cpt+h -T-35/20/10 -Z >grav_rmba.cpt
...
# 3. Calculate RMBA field with rheology-dependence mantle thermal model
  ...
  # (3) getAverageBox_TF_FZ; (4)getAverageBox_Ridge; (5) getAverageBox_ICOC    
    ...
    for i in {0..3}; do
        ...
        zmin_therm=`gmt grdinfo $grav_therm | grep "v_max" | awk '{printf "%.1f", $3}'`
        zmax_therm=`gmt grdinfo $grav_therm | grep "v_max" | awk '{printf "%.1f", $5}'`
        echo ${etaname} "Thermal - min: $zmin_therm mGal, max: $zmax_therm mGal"
        zmin_rmba=`gmt grdinfo $fullrmba | grep "v_max" | awk '{printf "%.1f", $3}'`
        zmax_rmba=`gmt grdinfo $fullrmba | grep "v_max" | awk '{printf "%.1f", $5}'`
...
# 4. Plot the figure
  ...
  # c. vp-hsc, vep-hsc, e. vep-vp    
    ...
    model3=disl
    model4=vp
      gmt begin Results/${dataname}_gravity_pattern_noshift pdf
        gmt subplot begin 4x4 -A+JTL+o0.5c -Fs15c/9c -M0.5c/1.5c -R$faa -JM15c -Ba1df30m -BWSne -T"Gravity anomaly estimated from rheology-dependence thermal models: "$dataname
            ...
            gmt subplot set 0,0
                ...
                gmt grdimage grav_${etaname}.nc -BwsEN -Cgrav_therm.cpt #-I${grav_therm_minusMeanValue}.grad
                gmt grdcontour grav_${etaname}.nc -C10
                gmt colorbar -DJCB+o0/0.5c -Bxaf+l"$modelname " -By+l"mGal" -Cgrav_therm.cpt
                plotControlTransformFault              
            gmt subplot set 0,1
                modelname=$model2
                etaname=${Etas[1]}
            ...
            gmt subplot set 1,0
                modelname=$model1
                etaname=${Etas[0]}
                rmba=Results/${dataname}_fullrmba_${etaname}.nc
                gmt basemap -BwseN -Ba 
                #makecpt_grd $rmba
                gmt grdimage $fullrmba -Cgrav_rmba.cpt #-I${rmba}.grad
                gmt colorbar -DJCB+o0/0.5c -Bxaf+l"RMBA ($modelname) " -By+l"mGal" -Cgrav_rmba.cpt #-G${data_min}/${data_max}
                plotControlTransformFault
            gmt subplot set 1,1
                modelname=$model2
                etaname=${Etas[1]}
